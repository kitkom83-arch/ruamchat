...
async function main() {
  ...
  const segment = await requestJson("POST", "/broadcasts/segments", {
    name: `Sprint 46 suppression ${Date.now()}`,
  ...
  const campaign = await requestJson("POST", "/broadcasts/campaigns", {
    name: `Sprint 46 suppression ${Date.now()}`,
    description: "Safe smoke campaign; provider outbound disabled; compliance filter drilldown API mode",
  ...
  const campaignCompliance = Array.isArray(complianceLogs)
    ...
    : null;
  const filteredByCampaign = await requestJson("GET", `/broadcasts/compliance-logs?${new URLSearchParams({
    campaignId: campaign.id,
    limit: "1",
    offset: "0"
  }).toString()}`);
  const filteredByReason = await requestJson("GET", `/broadcasts/compliance-logs?${new URLSearchParams({
    campaignId: campaign.id,
  ...
  const campaignFilteredRow = findComplianceRow(filteredByCampaign, campaign.id, contactId);
  const reasonFilteredRow = findComplianceRow(filteredByReason, campaign.id, contactId);
  record("broadcast compliance endpoint available in API", Array.isArray(complianceLogs) && Array.isArray(filteredByCampaign.items));
  record("broadcast compliance data is readable from backend", Boolean(campaignCompliance && campaignFilteredRow && reasonFilteredRow));
  record("filtered row exists", Boolean(reasonFilteredRow));
  record("campaignId filter returns matching row", campaignFilteredRow?.campaignId === campaign.id);
  record("reason filter returns do_not_contact row", reasonFilteredRow?.reason === "do_not_contact");
  record("broadcast compliance DTO context preserved", hasComplianceEndpointContext(reasonFilteredRow, campaign.id, room, conversation.id, contactId));
  record("pagination/limit is safe", filteredByCampaign.limit === 1 && filteredByCampaign.offset === 0 && filteredByCampaign.total >= 1 && filteredByCampaign.externalCalls === 0);
  record("broadcast compliance DTO has no token/secret fields", noRawSecretFields({ complianceLogs, filteredByCampaign, filteredByReason }));
  ...
  const aggregateDtos = { customer360Before, blockedState, preview, dryRun, sendNow, sendLogs, complianceLogs, filteredByCampaign, filteredByReason, auditLogs, restored, refetched };
...
function finish() {
  ...
  if (failed.length > 0) {
    throw new Error(`Sprint 46 smoke failed: ${failed.map((item) => item.name).join(", ")}`);
  }
}
...
function findComplianceRow(pageOrRows, campaignId, contactId) {
