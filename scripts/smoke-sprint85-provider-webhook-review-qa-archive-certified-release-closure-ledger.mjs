...
const runId = `sprint85-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
const results = [];
...
async function main() {
  ...
  const sprint85Source = {
    shared: sourceSlice(shared, "providerWebhookReviewQaHandoffReleaseClosureLedgerRowKeySchema", "providerWebhookUnmatchedInboundBulkReviewRequestSchema"),
    providerController: sourceSlice(providerController, "review-qa-handoff-bundle/locked-archive/finalization/release-evidence/verification/certification/closure-ledger", "review-closure-report/export"),
    providerService: [
      sourceSlice(providerService, "getReviewQaHandoffArchiveReleaseClosureLedger(", "private getLockedArchiveContext"),
      sourceSlice(providerService, "function assertQaHandoffArchiveReleaseClosureLedgerReady", "function assertQaHandoffArchiveReleaseAttestationAuditReady")
    ].join("\n"),
    apiClient: sourceSlice(apiClient, "getProviderWebhookReviewQaHandoffArchiveReleaseClosureLedger", "getProviderWebhookReviewClosureReportExport"),
    settingsData: sourceSlice(settingsData, "loadSettingsProviderWebhookReviewQaHandoffArchiveReleaseClosureLedgerData", "loadSettingsProviderWebhookReviewClosureReportRedactionAuditData"),
    settingsPage: sourceSlice(settingsPage, "loadReviewQaHandoffArchiveReleaseClosureLedger", "loadClosureReportRedactionAudit"),
    providerPanel: sourceSlice(providerPanel, "Load closure ledger", "Audit report export redaction")
  };

  record("smoke:sprint85 registered",
    rootPackage.scripts?.["smoke:sprint85"] === "node scripts/smoke-sprint85-provider-webhook-review-qa-archive-certified-release-closure-ledger.mjs"
  );
  record("Sprint 84/83/82/81/80/79/78/77/76/75 regression smoke scripts registered", [
  ...
  record("shared closure ledger DTO export",
    shared.includes("providerWebhookReviewQaHandoffReleaseClosureLedgerSchema") &&
    shared.includes("ProviderWebhookReviewQaHandoffReleaseClosureLedger") &&
    sprint85Source.shared.includes("externalCalls: z.literal(0)") &&
    sprint85Source.shared.includes("ledgerKind") &&
    sprint85Source.shared.includes("ledgerStatus") &&
    sprint85Source.shared.includes("releaseCertificationDigest") &&
    sprint85Source.shared.includes("ledgerRows") &&
    sprint85Source.shared.includes("prerequisiteChecklist") &&
    sprint85Source.shared.includes("certificationChecklist") &&
    sprint85Source.shared.includes("ledgerSummary")
  );
  record("backend closure ledger route registration",
    providerController.includes('@Get("review-qa-handoff-bundle/locked-archive/finalization/release-evidence/verification/certification/closure-ledger")') &&
    providerController.includes("getReviewQaHandoffArchiveReleaseClosureLedger")
  );
  record("service closure ledger implementation",
    providerService.includes("getReviewQaHandoffArchiveReleaseClosureLedger") &&
    providerService.includes("qaHandoffArchiveReleaseClosureLedgerResponse") &&
    providerService.includes("assertQaHandoffArchiveReleaseClosureLedgerReady") &&
    providerService.includes("releaseCertificationDigest") &&
    providerService.includes("ledgerRows") &&
    providerService.includes("certificationChecklistComplete") &&
    providerService.includes("externalCalls: 0 as const")
  );
  record("API client closure ledger wiring",
    apiClient.includes("getProviderWebhookReviewQaHandoffArchiveReleaseClosureLedger") &&
    apiClient.includes("providerWebhookReviewQaHandoffReleaseClosureLedgerSchema") &&
    apiClient.includes("/provider-webhooks/review-qa-handoff-bundle/locked-archive/finalization/release-evidence/verification/certification/closure-ledger")
  );
  record("settings-data closure ledger wiring",
    settingsData.includes("loadSettingsProviderWebhookReviewQaHandoffArchiveReleaseClosureLedgerData") &&
    settingsData.includes("getProviderWebhookReviewQaHandoffArchiveReleaseClosureLedger") &&
    settingsData.includes("createMockReviewQaHandoffArchiveReleaseClosureLedger")
  );
  record("provider readiness panel closure ledger control/result/error text",
    settingsPage.includes("QA Archive Release Closure Ledger API error") &&
    settingsPage.includes("onLoadReviewQaHandoffArchiveReleaseClosureLedger={loadReviewQaHandoffArchiveReleaseClosureLedger}") &&
    providerPanel.includes("Load closure ledger") &&
    providerPanel.includes("QA archive release closure ledger:") &&
    providerPanel.includes("ledgerStatus=") &&
    providerPanel.includes("releaseCertificationDigest=") &&
    providerPanel.includes("closureLedgerCheckedCount=")
  );
  record("no DATA_MODE=api mock/local fallback markers",
    /if \(mode === "api"\) \{\s*return \{\s*mode,\s*closureLedger: await getProviderWebhookReviewQaHandoffArchiveReleaseClosureLedger/s.test(settingsData) &&
    !/DATA_MODE=api[\s\S]{0,180}(mock|local|fallback)|(?:mock|local|fallback)[\s\S]{0,180}DATA_MODE=api/i.test(sprint85Source.settingsData)
  );
  record("static Sprint 85 source has no provider outbound send markers", !containsProviderOutbound(sprint85Source));
  record("static Sprint 85 source has no external notification send markers", !containsExternalNotification(sprint85Source));
  record("static Sprint 85 source has no AI/OpenAI call markers", !containsAiCall(sprint85Source));
  record("static Sprint 85 source has no raw provider material markers", safePayloadObject(sprint85Source));

  const health = await safeJson(await request("GET", "/health"));
  record("GET /health reachable", health?.status === "ok" && health?.service === "api");

  const ledgerItem = await createNoMatchItem("closure-ledger", "Safe Sprint 85 closure ledger target");
  ...
  const ledgerBeforeLock = await requestJson("GET", `/provider-webhooks/review-qa-handoff-bundle/locked-archive/finalization/release-evidence/verification/certification/closure-ledger?${filters}`);
  const receiptSignOff = await safeJson(await request("POST", `/provider-webhooks/review-qa-handoff-bundle/receipt/sign-off?${filters}`, {
    ...
    reviewerLabel: "safe sprint85 reviewer"
  }));
  const lock = await safeJson(await request("POST", `/provider-webhooks/review-qa-handoff-bundle/acceptance-lock?${filters}`, {
    lockReason: "Safe Sprint 85 QA archive release closure ledger accepted",
    acceptedByRole: "QA lead",
    acceptedByLabel: "safe sprint85 reviewer"
  }));
  const ledgerBeforeArchiveExport = await requestJson("GET", `/provider-webhooks/review-qa-handoff-bundle/locked-archive/finalization/release-evidence/verification/certification/closure-ledger?${filters}`);
  ...
  const stateBeforeRead = unmatchedItems(beforeReadPage).find((item) => item.id === ledgerItem.id);
  ...
  const stateAfterRead = unmatchedItems(afterReadPage).find((item) => item.id === ledgerItem.id);

  record("closure ledger requires acceptance lock before prerequisites", ledgerBeforeLock.status === 409 && /acceptance lock is required/i.test(JSON.stringify(ledgerBeforeLock.body)));
  record("closure ledger requires locked archive export", ledgerBeforeArchiveExport.status === 409 && /locked archive export is required/i.test(JSON.stringify(ledgerBeforeArchiveExport.body)));
  ...
  record("complete prerequisite chain through Sprint 84 certification", [receiptSignOff, lock, exportedArchive, manifest, integrity, retentionAudit, finalization, signOff, receipt, releaseEvidence, releaseVerification, releaseCertification].every(Boolean));
  record("closure ledger endpoint reachable", safeReleaseClosureLedgerShape(closureLedger));
  record("ledger rows present", Array.isArray(closureLedger.ledgerRows) && closureLedger.ledgerRows.length > 0 && closureLedger.ledgerRows.every(safeClosureLedgerRowShape));
  record("ledger status present", closureLedger.ledgerStatus === "certified_release_closed");
  record("certification status present", closureLedger.certificationStatus === "certified");
  record("digest chain status present", closureLedger.digestChainStatus === "confirmed");
  record("safe filename/digest present", typeof closureLedger.safeFilename === "string" && closureLedger.safeFilename.includes("release-closure-ledger") && typeof closureLedger.safeDigest === "string" && closureLedger.safeDigest.startsWith("sha256:"));
  record("release certification digest present", closureLedger.releaseCertificationDigest === releaseCertification.safeDigest);
  record("deterministic counts present", Number.isInteger(closureLedger.counts?.closureLedgerCheckedCount) && closureLedger.counts?.ledgerRowCount === closureLedger.ledgerRows.length && closureLedger.counts?.ledgerNeedsReviewRowCount === 0);
  record("externalCalls=0", closureLedger.externalCalls === 0);
  record("no mutation before/after closure ledger reads", metadataOnlyStateMatches(stateBeforeRead, stateAfterRead));
  ...
  record("no provider outbound", !containsProviderOutbound({ receiptSignOff, lock, archive, exportedArchive, manifest, integrity, retentionAudit, finalization, signOff, receipt, releaseEvidence, releaseVerification, releaseCertification, closureLedger }));
  record("no external notification", !containsExternalNotification({ receiptSignOff, lock, archive, exportedArchive, manifest, integrity, retentionAudit, finalization, signOff, receipt, releaseEvidence, releaseVerification, releaseCertification, closureLedger }));
  record("no AI/OpenAI call evidence", !containsAiCall({ receiptSignOff, lock, archive, exportedArchive, manifest, integrity, retentionAudit, finalization, signOff, receipt, releaseEvidence, releaseVerification, releaseCertification, closureLedger }));
  record("no raw provider material leakage", safePayloadObject({
    ledgerBeforeLock,
    ledgerBeforeArchiveExport,
...
async function createNoMatchItem(label, text) {
  ...
  const payload = linePayload(`safe-no-match-room-sprint85-${label}-${runId}`, `safe-sender-sprint85-${label}`, text);
...
function finish() {
  ...
  if (failed.length > 0) {
    console.error(`Sprint 85 smoke failed: ${failed.map((result) => result.name).join(", ")}`);
    process.exitCode = 1;
    return;
  }
  console.log("Sprint 85 smoke passed");
}
