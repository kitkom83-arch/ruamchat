...
const runId = `sprint76-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
const results = [];
...
async function main() {
  ...
  record("smoke:sprint76 registered",
    rootPackage.scripts?.["smoke:sprint76"] === "node scripts/smoke-sprint76-provider-webhook-review-qa-handoff-bundle-export.mjs"
  );
  record("Sprint 75/74/73/72/71 regression scripts registered", [
  ...
  record("shared QA handoff export DTO registered",
    shared.includes("providerWebhookReviewQaHandoffBundleExportSchema") &&
    shared.includes("ProviderWebhookReviewQaHandoffBundleExport") &&
    shared.includes("externalCalls: z.literal(0)") &&
    shared.includes("bundle: providerWebhookReviewQaHandoffBundleSchema")
  );
  record("backend QA handoff export route registered",
    providerController.includes('review-qa-handoff-bundle/export') &&
    providerController.includes("exportReviewQaHandoffBundle") &&
    providerController.includes("parseReviewClosureReportFilters")
  );
  record("service reuses read-only Sprint 75 bundle aggregation",
    providerService.includes("getReviewQaHandoffBundleExport") &&
    providerService.includes("this.getReviewQaHandoffBundle(tenantId, filters, actorUserId)") &&
    providerService.includes("safeDigestForExport(exportPayload)") &&
    !/getReviewQaHandoffBundleExport[\s\S]{0,2200}\b(create|update|delete|push|send|notify|record)\b/i.test(providerService)
  );
  record("readiness remains safe and externalCalls=0",
    readinessSource.includes("reviewExportQaHandoffEnabled") &&
    readinessSource.includes("externalCalls: 0")
  );
  record("API client export wiring registered",
    apiClient.includes("getProviderWebhookReviewQaHandoffBundleExport") &&
    apiClient.includes("/provider-webhooks/review-qa-handoff-bundle/export") &&
    apiClient.includes("providerWebhookReviewQaHandoffBundleExportSchema")
  );
  record("settings-data export wiring has API mode and distinguishable mock",
    settingsData.includes("exportSettingsProviderWebhookReviewQaHandoffBundleData") &&
    settingsData.includes("getProviderWebhookReviewQaHandoffBundleExport") &&
    settingsData.includes("createMockReviewQaHandoffBundleExport") &&
    settingsData.includes("provider-webhook-review-qa-handoff-bundle-export.json") &&
    settingsData.includes("sha256:mockqahandoffbundleexport")
  );
  record("provider UI export control/result/error text present",
    providerPanel.includes("Export QA handoff bundle") &&
    providerPanel.includes("QA handoff bundle export:") &&
    settingsPage.includes("QA Handoff Bundle Export API error") &&
    providerPanel.includes("reviewQaHandoffBundleExportError") &&
    settingsPage.includes("setReviewQaHandoffBundleExport(null)")
  );
  record("API-mode export has no silent mock/local fallback markers",
    apiClient.includes("request(`/provider-webhooks/review-qa-handoff-bundle/export") &&
    settingsData.includes('if (mode === "api")') &&
    settingsData.indexOf("getProviderWebhookReviewQaHandoffBundleExport") < settingsData.indexOf("createMockReviewQaHandoffBundleExport")
  );
  ...
  record("GET /health/readiness reachable", readinessBefore?.status === "ok" && safeReadinessSprint76Summary(readinessBefore?.providerReadiness));
  ...
  const evidenceItem = await createNoMatchItem("evidence", "Safe Sprint 76 QA handoff export target");
  ...
  const assigned = await safeJson(await request("PATCH", `/provider-webhooks/unmatched-inbound/${encodeURIComponent(evidenceItem.id)}/assignment`, {
    ...
    note: "Safe Sprint 76 QA handoff export assignment"
  }));
  const escalated = await safeJson(await request("PATCH", `/provider-webhooks/unmatched-inbound/${encodeURIComponent(evidenceItem.id)}/escalation`, {
    ...
    note: "Safe Sprint 76 QA handoff export escalation"
  }));
  const resolved = await safeJson(await request("PATCH", `/provider-webhooks/unmatched-inbound/${encodeURIComponent(evidenceItem.id)}/resolution`, {
    ...
    note: "Safe Sprint 76 QA handoff export resolution"
  }));
  ...
  const qaExport = await safeJson(await request("GET", `/provider-webhooks/review-qa-handoff-bundle/export?${filters}`));
  record("QA handoff bundle endpoint reachable", safeQaHandoffBundleShape(qaBundle));
  record("QA handoff export endpoint reachable", safeQaHandoffBundleExportShape(qaExport));
  record("QA handoff export response safe fields only", safePayloadObject(qaExport));
  record("QA handoff export safe filename present", qaExport.safeFilename === "provider-webhook-review-qa-handoff-bundle-export.json");
  record("QA handoff export safe digest present", typeof qaExport.safeDigest === "string" && qaExport.safeDigest.startsWith("sha256:"));
  record("QA handoff export deterministic status/counts fields",
    ["ready", "needs_review", "blocked"].includes(qaExport.status) &&
    Number.isInteger(qaExport.counts?.totalItems) &&
    Number.isInteger(qaExport.counts?.evidenceManifestCount) &&
    qaExport.bundle?.safeDigest === qaBundle.safeDigest
  );
  record("QA handoff export readiness flags present",
    qaExport.readinessFlags?.reviewExportQaHandoffEnabled === true &&
    qaExport.readinessFlags?.reviewClosureReportExportEnabled === true
  );
  record("QA handoff export summaries safe",
    qaExport.closureEvidenceSummary?.externalCalls === 0 &&
    qaExport.exportManifestSummary?.externalCalls === 0 &&
    qaExport.redactionAuditSummary?.externalCalls === 0 &&
    qaExport.integritySummary?.externalCalls === 0
  );
  record("QA handoff export manual checks pass safe guardrails",
    qaExport.manualQaChecks?.rawPayloadAbsent === true &&
    qaExport.manualQaChecks?.rawSignatureAbsent === true &&
    qaExport.manualQaChecks?.tokenAbsent === true &&
    qaExport.manualQaChecks?.replyTokenAbsent === true &&
    qaExport.manualQaChecks?.rawSenderIdAbsent === true &&
    qaExport.manualQaChecks?.rawRoomIdAbsent === true &&
    qaExport.manualQaChecks?.providerOutboundAbsent === true &&
    qaExport.manualQaChecks?.externalCallsZero === true
  );
  record("QA handoff export externalCalls=0", qaExport.externalCalls === 0 && noNonzeroExternalCalls(qaExport));
  ...
  record("QA handoff bundle/export reads do not mutate review/link/message state", metadataOnlyStateMatches(stateBeforeRead, stateAfterRead));

  const readinessAfter = await safeJson(await request("GET", "/health/readiness"));
  record("readiness after QA export remains externalCalls=0", safeReadinessSprint76Summary(readinessAfter?.providerReadiness) && noNonzeroExternalCalls(readinessAfter));
...
async function createNoMatchItem(label, text) {
  ...
  const payload = linePayload(`safe-no-match-room-sprint76-${label}-${runId}`, `safe-sender-sprint76-${label}`, text);
...
function safeReadinessSprint76Summary(value) {
  return value &&
...
function safeQaHandoffBundleShape(value) {
  return value &&
    ...
    value.readiness &&
    value.closureReportExport &&
    value.closureReportManifest &&
    value.closureReportRedactionAudit &&
    value.closureExportIntegrity &&
...
function safeQaHandoffBundleExportShape(value) {
  return value &&
    value.exportKind === "qa-handoff-bundle" &&
    value.format === "json" &&
    ...
    typeof value.safeDigest === "string" &&
    value.safeDigest.startsWith("sha256:") &&
    ["ready", "needs_review", "blocked"].includes(value.status) &&
    value.counts &&
    value.readinessFlags &&
    value.closureEvidenceSummary &&
    value.exportManifestSummary &&
    value.redactionAuditSummary &&
    value.integritySummary &&
    value.manualQaChecks &&
    safeQaHandoffBundleShape(value.bundle) &&
    value.externalCalls === 0;
}
...
function finish() {
  ...
  console.log(`Sprint 76 smoke checks: ${results.length - failed.length}/${results.length} passed`);
  if (failed.length > 0) {
