...
const runId = `sprint116-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
const results = [];
let liveChecks = 0;
...
function safeCustodyChainIntegrityLedgerReceiptShape(value) {
  return value &&
    value.receiptKind === "qa-handoff-locked-archive-certified-release-final-archive-seal-post-closure-preservation-custody-chain-integrity-ledger-receipt" &&
    ...
    value.safeFilename === "provider-webhook-certified-release-post-closure-preservation-custody-chain-integrity-ledger-receipt.json" &&
    /^sha256:[a-z0-9]+$/i.test(String(value.safeDigest ?? "")) &&
    value.postClosurePreservationCustodyChainIntegrityLedgerDigest === value.safeDigest &&
    /^sha256:[a-z0-9]+$/i.test(String(value.postClosurePreservationCustodyChainSealDigest ?? "")) &&
    value.inheritedPostClosurePreservationCustodyChainSealReceiptSummary?.postClosurePreservationCustodyChainSealStatus === "sealed" &&
    value.inheritedPostClosurePreservationCustodyChainSealReceiptSummary?.postClosurePreservationCustodyAuditStatus === "audited" &&
    value.inheritedPostClosurePreservationCustodyChainSealReceiptSummary?.postClosurePreservationContinuityLedgerStatus === "continuous" &&
    value.inheritedPostClosurePreservationCustodyChainSealReceiptSummary?.postClosurePreservationVerificationStatus === "verified" &&
    value.inheritedPostClosurePreservationCustodyChainSealReceiptSummary?.externalCallsZero === true &&
    ...
    Array.isArray(value.custodyChainIntegrityLedgerRows) &&
    value.custodyChainIntegrityLedgerRows.map((row) => row.sprintNumber).join(",") === "103,104,105,106,107,108,109,110,111,112,113,114,115,116" &&
    value.custodyChainIntegrityLedgerRows.every((row) =>
...
function stableCustodyChainIntegrityLedgerReceiptSnapshot(value) {
  ...
  return {
    ...stable,
    custodyChainIntegrityLedgerRows: Array.isArray(custodyChainIntegrityLedgerRows)
      ? custodyChainIntegrityLedgerRows.map(({ generatedAt, checkedAt, ...row }) => row)
      : custodyChainIntegrityLedgerRows
  };
}
...
async function prepareLiveArchiveChain(filters) {
  const payload = linePayload(`safe-no-match-room-sprint116-${runId}`, `safe-sender-sprint116-${runId}`, "Safe Sprint 116 custody chain integrity ledger receipt target");
  const created = await requestJson("POST", "/provider-webhooks/sandbox-events", {
    ...
    eventId: `${runId}-custody-chain-integrity-ledger`,
  ...
  await requestJson("POST", `/provider-webhooks/review-qa-handoff-bundle/receipt/sign-off?${filters}`, {
    ...
    reviewerLabel: "safe sprint116 reviewer"
  });
  await requestJson("POST", `/provider-webhooks/review-qa-handoff-bundle/acceptance-lock?${filters}`, {
    lockReason: "Safe Sprint 116 certified release custody chain integrity ledger receipt accepted",
    acceptedByRole: "QA lead",
    acceptedByLabel: "safe sprint116 reviewer"
  });
  ...
  await requestJson("POST", `${acceptanceRecordPath}?${filters}`, {
    ...
    acknowledgedByLabel: "safe sprint116 release owner",
    acknowledgedChecklistKeys
  });
  await requestJson("POST", `${noopExecutionDryRunPath}?${filters}`, {
    requestedBy: "safe sprint116 release owner",
    checklistAcknowledged: true,
    operatorNote: "Safe no-op execution dry-run from Sprint 116 smoke",
...
function linePayload(roomId, userIdValue, text) {
  return {
    destination: "safe-sprint116-destination",
    events: [{
      type: "message",
      mode: "active",
      timestamp: Date.now(),
...
async function main() {
  ...
  record("package script registration", packageJson.includes('"smoke:sprint116"') && packageJson.includes("smoke-sprint116-provider-webhook-certified-release-final-archive-seal-post-closure-preservation-custody-chain-integrity-ledger-receipt.mjs"));
  record("DTO/schema export", shared.includes("providerWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceiptSchema") && shared.includes("ProviderWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceipt") && shared.includes("postClosurePreservationCustodyChainIntegrityLedgerStatus") && shared.includes("postClosurePreservationCustodyChainIntegrityLedgerMutationCount: z.literal(0)") && shared.includes("externalCalls: z.literal(0)") && shared.includes("providerOutboundCallCount: z.literal(0)") && shared.includes("externalNotificationSendCount: z.literal(0)") && shared.includes("aiCallCount: z.literal(0)"));
  record("backend route tenant requirement", providerController.includes(`@Get("${endpointPath.slice("/provider-webhooks/".length)}")`) && providerController.includes("getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceipt(") && providerController.includes("requireTenantId(tenant)"));
  record("service derives from Sprint 115", serviceMethod.includes("getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt") && !serviceMethod.includes("getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyAuditReceipt(") && serviceResponse.includes("postClosurePreservationCustodyChainSealReceipt"));
  record("service fails closed on incomplete Sprint 115 prerequisites", serviceMethod.includes("ConflictException") && serviceMethod.includes("custody chain integrity ledger receipt prerequisites are incomplete") && providerService.includes("function certifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReady(") && providerService.includes("row.sprintNumber === 115"));
  record("API client wiring", apiClient.includes("getProviderWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceipt") && apiClient.includes(endpointPath) && apiClient.includes("providerWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceiptSchema"));
  record("settings-data API mode has no mock fallback", settingsDataLoader.includes('if (mode === "api")') && settingsDataLoader.indexOf("getProviderWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceipt") < settingsDataLoader.indexOf("createMockReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceipt"));
  record("mock/local safe fixture remains safe", settingsData.includes("createMockReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceipt") && settingsData.includes("mockqahandoffcertifiedreleasefinalarchivesealpostclosurepreservationcustodychainintegrityledgerreceipt") && settingsData.includes("provider-webhook-certified-release-post-closure-preservation-custody-chain-integrity-ledger-receipt.json") && !leaksRawProviderMaterial(sourceSlice(settingsData, "function createMockReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceipt(", "function createMockReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt(")));
  record("Settings > Channels Sprint 116 control/result/error text", providerPanel.includes("Load post-closure preservation custody chain integrity ledger receipt") && providerPanel.includes("QA archive certified release post-closure preservation custody chain integrity ledger receipt") && settingsPage.includes("Post-Closure Preservation Custody Chain Integrity Ledger Receipt API error"));
  record("stale Sprint 116 receipt clears on upstream/API failure", settingsPage.includes("clearReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceipt()") && pageHandler.includes("setReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceipt(null)") && pageHandler.includes("catch (reason)"));
  ...
  record("no raw provider material leakage markers", !leaksRawProviderMaterial(serviceResponse) && !leaksRawProviderMaterial(sourceSlice(providerPanel, "reviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceipt ? e", "reviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt ? e")));
  ...
  record("repeat read has no mutation test coverage", controllerTest.includes("postClosurePreservationCustodyChainIntegrityLedgerReceiptReadback") && controllerTest.includes("beforePostClosurePreservationCustodyChainIntegrityLedgerReceiptRead") && controllerTest.includes("afterPostClosurePreservationCustodyChainIntegrityLedgerReceiptRead"));
  record("digest continuity from Sprint 115", serviceResponse.includes("postClosurePreservationCustodyChainSealDigest: postClosurePreservationCustodyChainSealReceipt.postClosurePreservationCustodyChainSealDigest") && serviceResponse.includes("postClosurePreservationCustodyChainSealDigest: postClosurePreservationCustodyChainSealReceipt.safeDigest"));
  record("focused tests cover shared/API/web/settings/panel behavior", sharedTest.includes("postClosurePreservationCustodyChainIntegrityLedgerStatus") && controllerTest.includes("postClosurePreservationCustodyChainIntegrityLedgerReceipt") && apiClientTest.includes("getProviderWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceipt") && settingsDataTest.includes("loadSettingsProviderWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceiptData") && providerPanelTest.includes("Load post-closure preservation custody chain integrity ledger receipt"));
  ...
  try {
    ...
    record("live endpoint safe Sprint 116 shape", safeCustodyChainIntegrityLedgerReceiptShape(first.body));
    ...
    record("repeat live read has no mutation", JSON.stringify(stableCustodyChainIntegrityLedgerReceiptSnapshot(firstLive)) === JSON.stringify(stableCustodyChainIntegrityLedgerReceiptSnapshot(secondLive)));

    const invalidTenant = await getJson(url);
    liveChecks += 1;
    record("invalid tenant does not return mock fallback", invalidTenant.response.status >= 400 && !safeCustodyChainIntegrityLedgerReceiptShape(invalidTenant.body), `${invalidTenant.response.status}`);
  } catch (error) {
    record("live endpoint checks completed", false, error instanceof Error ? error.message : String(error));
  }

  const failed = results.filter((result) => !result.passed);
  console.log(`[sprint116-smoke] assertions=${results.length} liveChecks=${liveChecks} failed=${failed.length}`);
  if (liveChecks < 3) {
    console.error("[sprint116-smoke] live endpoint checks are mandatory and did not complete");
    process.exitCode = 1;
  }
  if (failed.length > 0) {
    process.exitCode = 1;
