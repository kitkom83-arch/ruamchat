...
async function main() {
  ...
  const segment = await requestJson("POST", "/broadcasts/segments", {
    name: `Sprint 44 suppression ${Date.now()}`,
  ...
  const campaign = await requestJson("POST", "/broadcasts/campaigns", {
    name: `Sprint 44 suppression ${Date.now()}`,
    description: "Safe smoke campaign; provider outbound disabled",
  ...
  const aggregateDtos = { customer360Before, blockedState, preview, dryRun, sendNow, sendLogs, auditLogs, restored, refetched };
...
function finish() {
  ...
  if (failed.length > 0) {
    throw new Error(`Sprint 44 smoke failed: ${failed.map((item) => item.name).join(", ")}`);
  }
}
