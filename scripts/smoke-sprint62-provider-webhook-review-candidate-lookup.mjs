...
async function main() {
  ...
  record("smoke script registered", rootPackage.scripts?.["smoke:sprint62"] === "node scripts/smoke-sprint62-provider-webhook-review-candidate-lookup.mjs");
  for (const sprint of [61, 60, 59, 58, 57, 56, 55, 54, 53]) {
    record(`Sprint ${sprint} regression script registered`, typeof rootPackage.scripts?.[`smoke:sprint${sprint}`] === "string");
  }
  record("API client wires candidate lookup with tenant header", apiClient.includes("getProviderWebhookUnmatchedInboundCandidates") && apiClient.includes("/candidates") && apiClient.includes("\"x-tenant-id\": getApiTenantId()"));
  record("API client wires safe filter params", apiClient.includes("providerWebhookUnmatchedInboundFiltersSchema.parse") && apiClient.includes("new URLSearchParams()") && apiClient.includes("getProviderWebhookUnmatchedInbound(filters"));
  ...
  record("GET /health/readiness reachable", readinessBefore?.status === "ok" && safeReadinessSprint62Summary(readinessBefore?.providerReadiness));
  ...
  const noMatchItem = await createNoMatchItem("filters", "Safe Sprint 62 filtered inbound");
  record("POST valid signed sandbox no-match event created unmatched item", Boolean(noMatchItem?.id) && noMatchItem.unmatchedStatus === "review-needed");

  const filtered = await safeJson(await request("GET", "/provider-webhooks/unmatched-inbound?provider=line&reviewStatus=pending&linkStatus=none&status=open&eventType=message.created&limit=10"));
  record("GET unmatched list with safe filters reachable", Array.isArray(filtered) && filtered.some((item) => item.id === noMatchItem.id));
  record("filter response safe", Array.isArray(filtered) && filtered.every(safeUnmatchedItemShape) && noRawSecretFields(filtered) && noRawPayloadValues(filtered));

  const emptyCandidates = await safeJson(await request("GET", `/provider-webhooks/unmatched-inbound/${encodeURIComponent(noMatchItem.id)}/candidates`));
  record("GET candidate lookup endpoint reachable", Array.isArray(emptyCandidates));
  record("candidate response safe", Array.isArray(emptyCandidates) && emptyCandidates.every(safeCandidateShape) && noRawSecretFields(emptyCandidates) && noRawPayloadValues(emptyCandidates));

  const unsafeLink = await request("POST", `/provider-webhooks/unmatched-inbound/${encodeURIComponent(noMatchItem.id)}/link-conversation`, {
  ...
  if (prepared) {
    const candidateItem = await createNoMatchItem("candidate", "Safe Sprint 62 candidate inbound", prepared.roomKey, "dry-run");
    const candidates = await safeJson(await request("GET", `/provider-webhooks/unmatched-inbound/${encodeURIComponent(candidateItem.id)}/candidates`));
    const selected = Array.isArray(candidates) ? candidates.find((candidate) => candidate.conversationId === prepared.conversationId) : null;
    record("safe existing conversation candidate returned", Boolean(selected) && safeCandidateShape(selected));
    record("candidate preserves platform channel and room digest", selected?.platform === "line" && selected?.channelAccountId === "sandbox:line" && selected?.roomIdDigest?.startsWith("sha256:"));
    record("candidate lookup safe fields only", noRawSecretFields(candidates) && noRawPayloadValues(candidates));

    if (selected) {
      const linkOnly = await safeJson(await request("POST", `/provider-webhooks/unmatched-inbound/${encodeURIComponent(candidateItem.id)}/link-conversation`, {
        conversationId: selected.conversationId,
        actionMode: "link-only"
      }));
      record("link-only selected candidate marks linked without message persist", linkOnly?.linkStatus === "linked" && linkOnly?.messagePersisted === false && linkOnly?.linkedConversationId === selected.conversationId);

      const persistItem = await createNoMatchItem("candidate-persist", "Safe Sprint 62 persisted linked inbound", prepared.roomKey, "dry-run");
      ...
      record("link-and-persist selected candidate persists exactly one safe message", persisted?.linkStatus === "linked-message-persisted" && persisted?.messagePersisted === true && afterMessages.length === beforeMessages.length + 1);
      record("repeat link-and-persist creates no duplicate safe message", duplicate?.id === persisted?.id && afterDuplicateMessages.length === afterMessages.length);
  ...
  record("readiness counts updated safely", safeReadinessSprint62Summary(readinessAfter?.providerReadiness)
    && readinessAfter.providerReadiness.unmatchedInboundQueuedCount >= (readinessBefore?.providerReadiness?.unmatchedInboundQueuedCount ?? 0) + 1);
  record("readiness after actions externalCalls=0", noNonzeroExternalCalls(readinessAfter));
  record("no provider outbound", !containsProviderOutbound({ readinessBefore, beforeUnmatched, filtered, unsafeLinkBody, afterEvents, readinessAfter }));
  record("no live provider network evidence", noLiveProviderNetworkEvidence({ readinessBefore, beforeUnmatched, filtered, unsafeLinkBody, afterEvents, readinessAfter }));

  finish();
}

async function createNoMatchItem(label, text, roomKey = `safe-no-match-room-sprint62-${label}-${Date.now()}`, inboundPersistenceMode = "sandbox-persist") {
  const eventId = `sprint62-${label}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  const payload = linePayload(roomKey, `safe-sender-sprint62-${label}`, text);
...
async function prepareSafeConversationRoute() {
  ...
  const roomKey = `safe-link-room-sprint62-${suffix}`;
  const conversationId = `sprint62-conversation-${suffix}`;
  try {
    await prisma.tenant.upsert({
      where: { id: tenantId },
      update: {},
      create: { id: tenantId, name: "Sprint 62 Safe Smoke Tenant" }
    });
    ...
    const contact = await prisma.contact.create({
      data: { tenantId, displayName: `Sprint 62 Smoke Contact ${suffix}` }
    });
    const identity = await prisma.contactIdentity.create({
      data: {
        ...
        platform: "line",
        channelAccountId,
        externalUserId: `safe-sprint62-identity-${suffix}`,
        displayName: "Sprint 62 Smoke Identity"
      }
    });
...
function linePayload(roomId, userId, text) {
  return {
    events: [{
      ...
      message: { id: "safe-message-sprint62", type: "text", text }
    }]
  };
}
...
function safeCandidateShape(value) {
  if (!value || typeof value !== "object") return false;
  const allowed = new Set([
    "conversationId", "platform", "channelAccountId", "roomIdDigest", "safeRoomLabel",
    "latestMessagePreview", "latestMessageAt", "matchReason", "matchConfidence", "externalCalls"
  ]);
  return Object.keys(value).every((key) => allowed.has(key))
    && typeof value.conversationId === "string"
    && typeof value.channelAccountId === "string"
    && /^sha256:/.test(value.roomIdDigest)
    && value.externalCalls === 0;
}

function safeReadinessSprint62Summary(readiness) {
...
function noRawPayloadValues(value) {
  return !/safe-message-sprint62|safe-no-match-room-sprint62|safe-link-room-sprint62|safe-sender-sprint62/i.test(JSON.stringify(value ?? {}));
}
...
function finish() {
  ...
  if (failed.length > 0) {
    throw new Error(`Sprint 62 smoke failed: ${failed.map((item) => item.name).join(", ")}`);
  }
}
