...
const runId = `sprint115-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
const results = [];
let liveChecks = 0;
...
function safeCustodyChainSealReceiptShape(value) {
  return value &&
    value.receiptKind === "qa-handoff-locked-archive-certified-release-final-archive-seal-post-closure-preservation-custody-chain-seal-receipt" &&
    ...
    value.safeFilename === "provider-webhook-certified-release-post-closure-preservation-custody-chain-seal-receipt.json" &&
    /^sha256:[a-z0-9]+$/i.test(String(value.safeDigest ?? "")) &&
    value.postClosurePreservationCustodyChainSealDigest === value.safeDigest &&
    /^sha256:[a-z0-9]+$/i.test(String(value.postClosurePreservationCustodyAuditDigest ?? "")) &&
    value.inheritedPostClosurePreservationCustodyAuditReceiptSummary?.postClosurePreservationCustodyAuditStatus === "audited" &&
    value.inheritedPostClosurePreservationCustodyAuditReceiptSummary?.postClosurePreservationContinuityLedgerStatus === "continuous" &&
    value.inheritedPostClosurePreservationCustodyAuditReceiptSummary?.externalCallsZero === true &&
    ...
    Array.isArray(value.custodyChainSealRows) &&
    value.custodyChainSealRows.map((row) => row.sprintNumber).join(",") === "103,104,105,106,107,108,109,110,111,112,113,114,115" &&
    value.custodyChainSealRows.every((row) =>
...
function stableCustodyChainSealReceiptSnapshot(value) {
  ...
  return {
    ...stable,
    custodyChainSealRows: Array.isArray(custodyChainSealRows)
      ? custodyChainSealRows.map(({ generatedAt, checkedAt, ...row }) => row)
      : custodyChainSealRows
  };
}
...
async function prepareLiveArchiveChain(filters) {
  const payload = linePayload(`safe-no-match-room-sprint115-${runId}`, `safe-sender-sprint115-${runId}`, "Safe Sprint 115 custody chain seal receipt target");
  const created = await requestJson("POST", "/provider-webhooks/sandbox-events", {
    ...
    eventId: `${runId}-custody-chain-seal`,
  ...
  await requestJson("POST", `/provider-webhooks/review-qa-handoff-bundle/receipt/sign-off?${filters}`, {
    ...
    reviewerLabel: "safe sprint115 reviewer"
  });
  await requestJson("POST", `/provider-webhooks/review-qa-handoff-bundle/acceptance-lock?${filters}`, {
    lockReason: "Safe Sprint 115 certified release custody chain seal receipt accepted",
    acceptedByRole: "QA lead",
    acceptedByLabel: "safe sprint115 reviewer"
  });
  ...
  await requestJson("POST", `${acceptanceRecordPath}?${filters}`, {
    ...
    acknowledgedByLabel: "safe sprint115 release owner",
    acknowledgedChecklistKeys
  });
  await requestJson("POST", `${noopExecutionDryRunPath}?${filters}`, {
    requestedBy: "safe sprint115 release owner",
    checklistAcknowledged: true,
    operatorNote: "Safe no-op execution dry-run from Sprint 115 smoke",
...
function linePayload(roomId, userIdValue, text) {
  return {
    destination: "safe-sprint115-destination",
    events: [{
      type: "message",
      mode: "active",
      timestamp: Date.now(),
...
async function main() {
  ...
  record("package script registration", packageJson.includes('"smoke:sprint115"') && packageJson.includes("smoke-sprint115-provider-webhook-certified-release-final-archive-seal-post-closure-preservation-custody-chain-seal-receipt.mjs"));
  record("DTO/schema export", shared.includes("providerWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceiptSchema") && shared.includes("ProviderWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt") && shared.includes("postClosurePreservationCustodyChainSealStatus") && shared.includes("externalCalls: z.literal(0)") && shared.includes("providerOutboundCallCount: z.literal(0)") && shared.includes("externalNotificationSendCount: z.literal(0)") && shared.includes("aiCallCount: z.literal(0)"));
  record("backend route tenant requirement", providerController.includes(`@Get("${endpointPath.slice("/provider-webhooks/".length)}")`) && providerController.includes("getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt(") && providerController.includes("requireTenantId(tenant)"));
  record("service derives from Sprint 114", serviceMethod.includes("getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyAuditReceipt") && !serviceMethod.includes("getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationContinuityLedgerReceipt(") && serviceResponse.includes("postClosurePreservationCustodyAuditReceipt"));
  record("service fails closed on incomplete Sprint 114 prerequisites", serviceMethod.includes("ConflictException") && serviceMethod.includes("custody chain seal receipt prerequisites are incomplete") && providerService.includes("function certifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReady(") && providerService.includes("postClosurePreservationCustodyAuditStatus === \"audited\"") && providerService.includes("row.sprintNumber === 114"));
  record("API client wiring", apiClient.includes("getProviderWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt") && apiClient.includes(endpointPath) && apiClient.includes("providerWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceiptSchema"));
  record("settings-data API mode has no mock fallback", settingsDataLoader.includes('if (mode === "api")') && settingsDataLoader.indexOf("getProviderWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt") < settingsDataLoader.indexOf("createMockReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt"));
  record("mock/local safe fixture remains safe", settingsData.includes("createMockReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt") && settingsData.includes("mockqahandoffcertifiedreleasefinalarchivesealpostclosurepreservationcustodychainsealreceipt") && settingsData.includes("provider-webhook-certified-release-post-closure-preservation-custody-chain-seal-receipt.json") && !leaksRawProviderMaterial(sourceSlice(settingsData, "function createMockReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt(", "function mockCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReady(")));
  record("Settings > Channels Sprint 115 control/result/error text", providerPanel.includes("Load post-closure preservation custody chain seal receipt") && providerPanel.includes("QA archive certified release post-closure preservation custody chain seal receipt") && settingsPage.includes("Post-Closure Preservation Custody Chain Seal Receipt API error"));
  record("stale Sprint 115 receipt clears on upstream/API failure", settingsPage.includes("clearReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt()") && pageHandler.includes("setReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt(null)") && pageHandler.includes("catch (reason)"));
  ...
  record("no raw provider material leakage markers", !leaksRawProviderMaterial(serviceResponse) && !leaksRawProviderMaterial(sourceSlice(providerPanel, "reviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt ? e", "reviewQaHandoffLockedArchive ? e")));
  ...
  record("repeat read has no mutation test coverage", controllerTest.includes("postClosurePreservationCustodyChainSealReceiptReadback") && controllerTest.includes("beforePostClosurePreservationCustodyChainSealReceiptRead") && controllerTest.includes("afterPostClosurePreservationCustodyChainSealReceiptRead"));
  record("digest continuity from Sprint 114", serviceResponse.includes("postClosurePreservationCustodyAuditDigest: postClosurePreservationCustodyAuditReceipt.postClosurePreservationCustodyAuditDigest") && serviceResponse.includes("postClosurePreservationCustodyAuditDigest: postClosurePreservationCustodyAuditReceipt.safeDigest"));
  record("focused tests cover shared/API/web/settings/panel behavior", sharedTest.includes("postClosurePreservationCustodyChainSealStatus") && controllerTest.includes("postClosurePreservationCustodyChainSealReceipt") && apiClientTest.includes("getProviderWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt") && settingsDataTest.includes("loadSettingsProviderWebhookReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceiptData") && providerPanelTest.includes("Load post-closure preservation custody chain seal receipt"));
  ...
  try {
    ...
    record("live endpoint safe Sprint 115 shape", safeCustodyChainSealReceiptShape(first.body));
    ...
    record("repeat live read has no mutation", JSON.stringify(stableCustodyChainSealReceiptSnapshot(firstLive)) === JSON.stringify(stableCustodyChainSealReceiptSnapshot(secondLive)));

    const invalidTenant = await getJson(url);
    liveChecks += 1;
    record("invalid tenant does not return mock fallback", invalidTenant.response.status >= 400 && !safeCustodyChainSealReceiptShape(invalidTenant.body), `${invalidTenant.response.status}`);
  } catch (error) {
    record("live endpoint checks completed", false, error instanceof Error ? error.message : String(error));
  }

  const failed = results.filter((result) => !result.passed);
  console.log(`[sprint115-smoke] assertions=${results.length} liveChecks=${liveChecks} failed=${failed.length}`);
  if (liveChecks < 3) {
    console.error("[sprint115-smoke] live endpoint checks are mandatory and did not complete");
    process.exitCode = 1;
  }
  if (failed.length > 0) {
    process.exitCode = 1;
