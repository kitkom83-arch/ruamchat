...
const runId = `sprint66-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
const results = [];
...
async function main() {
  ...
  record("smoke script registered", rootPackage.scripts?.["smoke:sprint66"] === "node scripts/smoke-sprint66-provider-webhook-review-queue-sla-alerts.mjs");
  for (const sprint of [65, 64, 63, 62, 61, 60, 59, 58, 57, 56, 55, 54, 53]) {
    record(`Sprint ${sprint} regression script registered`, typeof rootPackage.scripts?.[`smoke:sprint${sprint}`] === "string");
  }
  record("shared safe alerts DTO registered", shared.includes("providerWebhookReviewAlertsSchema") && shared.includes("providerWebhookReviewAlertItemSchema"));
  record("backend review alerts endpoint registered", providerController.includes("review-alerts") && providerController.includes("getReviewAlerts"));
  record("service implements safe queue SLA alerts", providerService.includes("getReviewAlerts") && providerService.includes("reviewAlertThresholds") && providerService.includes("reviewAlertItemFromUnmatched"));
  record("readiness exposes safe alert capability flags", readiness.includes("webhookReviewAlertsEnabled") && readiness.includes("webhookReviewQueueHealthEnabled") && readiness.includes("reviewAlertCriticalCount"));
  record("API client sends alerts requests", apiClient.includes("getProviderWebhookReviewAlerts") && apiClient.includes("/provider-webhooks/review-alerts"));
  record("settings data keeps API alerts backend-only", settingsData.includes("getProviderWebhookReviewAlerts(filters)") && settingsData.includes("createMockReviewAlerts"));
  record("provider UI renders queue SLA alerts controls", providerPanel.includes("Queue SLA alerts") && providerPanel.includes("Open diagnostics") && providerPanel.includes("SLA/staleness thresholds"));
  ...
  record("GET /health/readiness reachable", readinessBefore?.status === "ok" && safeReadinessSprint66Summary(readinessBefore?.providerReadiness));
  ...
  record("review alerts thresholds present", safeAlertsShape(alertsBefore) && alertsBefore.thresholds.staleWarningHours === 24 && alertsBefore.thresholds.staleCriticalHours === 72 && alertsBefore.thresholds.overSlaHours === 48);
  ...
  const reviewItem = await createNoMatchItem("review-one", "Safe Sprint 66 alert review one");
  const skipItem = await createNoMatchItem("skip-one", "Safe Sprint 66 alert skip two");
  ...
  record("review alerts after create safe", safeAlertsShape(alertsAfterCreate) && noRawSecretFields(alertsAfterCreate) && noRawPayloadValues(alertsAfterCreate) && noNonzeroExternalCalls(alertsAfterCreate));

  const filteredAlerts = await safeJson(await request("GET", "/provider-webhooks/review-alerts?provider=line&reviewStatus=pending&linkStatus=none&unmatchedStatus=review-needed&eventType=message.created&severity=info&receivedAtFrom=2026-01-01T00%3A00%3A00.000Z"));
  record("GET review alerts with safe filters reachable", safeAlertsShape(filteredAlerts) && filteredAlerts.appliedFilters.provider === "line" && filteredAlerts.appliedFilters.severity === "info");
  record("review alerts filters are safe", safeAlertsShape(filteredAlerts) && noRawSecretFields(filteredAlerts.appliedFilters) && noRawPayloadValues(filteredAlerts.appliedFilters));
  record("review alerts group fields present", safeAlertsShape(filteredAlerts) && safeCountRows(filteredAlerts.byProvider) && safeCountRows(filteredAlerts.byPlatform) && safeCountRows(filteredAlerts.byEventType) && safeCountRows(filteredAlerts.bySeverity));
  ...
  const reviewed = await safeJson(await request("PATCH", `/provider-webhooks/unmatched-inbound/${encodeURIComponent(reviewItem.id)}/review`, {
    status: "reviewed",
    reason: "safe sprint 66 single review"
  }));
  ...
  const bulkSkipped = await safeJson(await request("PATCH", "/provider-webhooks/unmatched-inbound/bulk-review", {
    ...
    reason: "safe sprint 66 bulk skip"
  }));
  ...
  record("readiness confirms alerts and externalCalls=0", safeReadinessSprint66Summary(readinessAfter?.providerReadiness) && noNonzeroExternalCalls(readinessAfter));
  record("no provider outbound", !containsProviderOutbound({ health, readinessBefore, metricsBefore, alertsBefore, beforeUnmatched, unmatchedAfterCreate, metricsAfterCreate, alertsAfterCreate, filteredAlerts, diagnosticsBeforeReview, reviewed, bulkSkipped, metricsAfterReview, alertsAfterReview, diagnosticsAfterReview, history, exported, afterEvents, readinessAfter }));
  record("no live provider network evidence", noLiveProviderNetworkEvidence({ health, readinessBefore, metricsBefore, alertsBefore, beforeUnmatched, unmatchedAfterCreate, metricsAfterCreate, alertsAfterCreate, filteredAlerts, diagnosticsBeforeReview, reviewed, bulkSkipped, metricsAfterReview, alertsAfterReview, diagnosticsAfterReview, history, exported, afterEvents, readinessAfter }));
...
async function createNoMatchItem(label, text) {
  ...
  const payload = linePayload(`safe-no-match-room-sprint66-${label}-${runId}`, `safe-sender-sprint66-${label}`, text);
...
function linePayload(roomId, userId, text) {
  return {
    events: [{
      ...
      replyToken: "raw-reply-token-sprint66",
      source: { type: "room", userId, roomId },
      message: { id: "safe-message-sprint66", type: "text", text }
    }]
  };
}
...
function safeReadinessSprint66Summary(readiness) {
...
function noRawPayloadValues(value) {
  return !/safe-message-sprint66|safe-no-match-room-sprint66|safe-sender-sprint66|raw-|reply-token/i.test(JSON.stringify(value ?? {}));
}
...
function finish() {
  ...
  if (failed.length > 0) {
    throw new Error(`Sprint 66 smoke failed: ${failed.map((item) => item.name).join(", ")}`);
  }
}
