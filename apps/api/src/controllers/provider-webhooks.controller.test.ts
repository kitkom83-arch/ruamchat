...
describe("ProviderWebhooksController sandbox events", () => {
  it("rejects missing tenant ids", async () => {
    ...
    expect(() => controller.getReviewQaHandoffCertifiedReleaseOperationsHandoffAcceptanceReceipt(undefined, {}, undefined)).toThrow(BadRequestException);
    expect(() => controller.getReviewQaHandoffCertifiedReleaseOperationsCustodyMonitoringReadinessLedger(undefined, {}, undefined)).toThrow(BadRequestException);
    expect(() => controller.getReviewQaHandoffCertifiedReleaseOperationsCustodyMonitoringCloseoutSealReceipt(undefined, {}, undefined)).toThrow(BadRequestException);
    ...
    expect(() => controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt(undefined, {}, undefined)).toThrow(BadRequestException);
    expect(() => controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt(undefined, {}, undefined)).toThrow(BadRequestException);
    expect(() => controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt(undefined, {}, undefined)).toThrow(BadRequestException);
  });
  ...
  it("returns safe QA archive finalization and retention sign-off without mutating review state", async () => {
    ...
    expect(() => controller.getReviewQaHandoffCertifiedReleaseHandoffAcceptanceRecord(tenantId, filters, "operator-current"))
      .toThrow("locked archive export is required before release evidence");
    expect(() => controller.getReviewQaHandoffCertifiedReleaseOperationsHandoffAcceptanceReceipt(tenantId, filters, "operator-current"))
      .toThrow("locked archive export is required before release evidence");
    expect(() => controller.getReviewQaHandoffCertifiedReleaseOperationsCustodyMonitoringReadinessLedger(tenantId, filters, "operator-current"))
      .toThrow("locked archive export is required before release evidence");
    expect(() => controller.getReviewQaHandoffCertifiedReleaseOperationsCustodyMonitoringCloseoutSealReceipt(tenantId, filters, "operator-current"))
      .toThrow("locked archive export is required before release evidence");
    expect(() => controller.getReviewQaHandoffCertifiedReleaseFinalNoExecutionEvidenceRollup(tenantId, filters, "operator-current"))
      .toThrow("locked archive export is required before release evidence");
    ...
    expect(() => controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationVerificationReceipt(tenantId, filters, "operator-current"))
      .toThrow("locked archive export is required before release evidence");
    expect(() => controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyAuditReceipt(tenantId, filters, "operator-current"))
      .toThrow("locked archive export is required before release evidence");
    expect(() => controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt(tenantId, filters, "operator-current"))
      .toThrow("locked archive export is required before release evidence");
    ...
    const afterOperatorCommandReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforeGoLiveAuthorizationReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    ...
    const afterLaunchWindowConfirmationReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforeGoLiveHoldReleaseAuthorizationReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    ...
    const afterOperationsHandoffReadinessPacketRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforeOperationsHandoffAcceptanceReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const operationsHandoffAcceptanceReceipt = controller.getReviewQaHandoffCertifiedReleaseOperationsHandoffAcceptanceReceipt(tenantId, filters, "operator-current");
    const afterOperationsHandoffAcceptanceReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforeOperationsCustodyMonitoringReadinessLedgerRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const operationsCustodyMonitoringReadinessLedger = controller.getReviewQaHandoffCertifiedReleaseOperationsCustodyMonitoringReadinessLedger(tenantId, filters, "operator-current");
    const afterOperationsCustodyMonitoringReadinessLedgerRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforeOperationsCustodyMonitoringCloseoutSealReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const operationsCustodyMonitoringCloseoutSealReceipt = controller.getReviewQaHandoffCertifiedReleaseOperationsCustodyMonitoringCloseoutSealReceipt(tenantId, filters, "operator-current");
    const afterOperationsCustodyMonitoringCloseoutSealReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforeFinalNoExecutionEvidenceRollupRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const finalNoExecutionEvidenceRollup = controller.getReviewQaHandoffCertifiedReleaseFinalNoExecutionEvidenceRollup(tenantId, filters, "operator-current");
    const afterFinalNoExecutionEvidenceRollupRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforeFinalEvidenceIndexRegressionGuardrailReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const finalEvidenceIndexRegressionGuardrailReceipt = controller.getReviewQaHandoffCertifiedReleaseFinalEvidenceIndexRegressionGuardrailReceipt(tenantId, filters, "operator-current");
    const finalEvidenceIndexRegressionGuardrailReceiptReadback = controller.getReviewQaHandoffCertifiedReleaseFinalEvidenceIndexRegressionGuardrailReceipt(tenantId, filters, "operator-current");
    const afterFinalEvidenceIndexRegressionGuardrailReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforeFinalArchiveSealOperationalClosureReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const finalArchiveSealOperationalClosureReceipt = controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealOperationalClosureReceipt(tenantId, filters, "operator-current");
    ...
    const afterPostClosurePreservationVerificationReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforePostClosurePreservationContinuityLedgerReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const postClosurePreservationContinuityLedgerReceipt = controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationContinuityLedgerReceipt(tenantId, filters, "operator-current");
    const postClosurePreservationContinuityLedgerReceiptReadback = controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationContinuityLedgerReceipt(tenantId, filters, "operator-current");
    const afterPostClosurePreservationContinuityLedgerReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforePostClosurePreservationCustodyAuditReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const postClosurePreservationCustodyAuditReceipt = controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyAuditReceipt(tenantId, filters, "operator-current");
    const postClosurePreservationCustodyAuditReceiptReadback = controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyAuditReceipt(tenantId, filters, "operator-current");
    const afterPostClosurePreservationCustodyAuditReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforePostClosurePreservationCustodyChainSealReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const postClosurePreservationCustodyChainSealReceipt = controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainSealReceipt(tenantId, filters, "operator-current");
    ...
    const afterPostClosurePreservationCustodyChainIntegrityLedgerReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforePostClosurePreservationCustodyChainIntegrityLedgerContinuityReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt = controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt(tenantId, filters, "operator-current");
    const postClosurePreservationCustodyChainIntegrityLedgerContinuityReceiptReadback = controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt(tenantId, filters, "operator-current");
    const afterPostClosurePreservationCustodyChainIntegrityLedgerContinuityReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforePostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt = controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt(tenantId, filters, "operator-current");
    const postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceiptReadback = controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt(tenantId, filters, "operator-current");
    const afterPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const beforePostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    const postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt = controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt(tenantId, filters, "operator-current");
    const postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceiptReadback = controller.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt(tenantId, filters, "operator-current");
    const afterPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceiptRead = listUnmatchedItems(controller, tenantId, { limit: 25 })
      .find((candidate) => candidate.id === item.id);
    ...
    expect(goLiveAuthorizationReceipt).toMatchObject({
    ...
    expect(goLiveAuthorizationReceipt.safeFilename).toBe("provider-webhook-review-qa-handoff-certified-release-go-live-authorization-receipt.json");
    expect(goLiveAuthorizationReceipt.goLiveAuthorizationReceiptDigest).toBe(goLiveAuthorizationReceipt.safeDigest);
    expect(goLiveAuthorizationReceipt.operatorCommandReceiptDigest).toBe(operatorCommandReceipt.operatorCommandReceiptDigest);
    expect(goLiveAuthorizationReceipt.goLiveAuthorizationReceiptRows.every((entry) => entry.complete && entry.goLiveAuthorizationReceiptStatus === "issued" && entry.launchWindowStatus === "ready")).toBe(true);
    expect(goLiveAuthorizationReceipt.launchWindowRows.every((entry) => entry.complete && entry.launchWindowStatus === "ready")).toBe(true);
    expect(goLiveAuthorizationReceipt.safeLaunchWindowRows.every((entry) => entry.complete && entry.safeLaunchWindowStatus === "ready")).toBe(true);
    expect(goLiveAuthorizationReceipt.inheritedOperatorCommandSummary).toMatchObject({
      operatorCommandReceiptStatus: "issued",
      goLiveAuthorizationStatus: "ready",
    ...
    expect(goLiveAuthorizationReceipt.counts.goLiveAuthorizationReceiptCheckedCount).toBe(1);
    expect(goLiveAuthorizationReceipt.counts.goLiveAuthorizationReceiptMutationCount).toBe(0);
    ...
    expect(goLiveHoldReleaseAuthorizationReceipt).toMatchObject({
    ...
    expect(goLiveHoldReleaseAuthorizationReceipt.safeFilename).toBe("provider-webhook-review-qa-handoff-certified-release-go-live-hold-release-authorization-receipt.json");
    expect(goLiveHoldReleaseAuthorizationReceipt.goLiveHoldReleaseAuthorizationReceiptDigest).toBe(goLiveHoldReleaseAuthorizationReceipt.safeDigest);
    expect(goLiveHoldReleaseAuthorizationReceipt.launchWindowConfirmationReceiptDigest).toBe(launchWindowConfirmationReceipt.launchWindowConfirmationReceiptDigest);
    expect(goLiveHoldReleaseAuthorizationReceipt.goLiveHoldReleaseAuthorizationRows.every((entry) => entry.complete && entry.goLiveHoldReleaseAuthorizationStatus === "authorized" && entry.launchApprovalStatus === "ready")).toBe(true);
    expect(goLiveHoldReleaseAuthorizationReceipt.launchApprovalRows.every((entry) => entry.complete && entry.goLiveHoldReleaseAuthorizationStatus === "authorized" && entry.launchApprovalStatus === "ready")).toBe(true);
    expect(goLiveHoldReleaseAuthorizationReceipt.inheritedLaunchWindowConfirmationSummary).toMatchObject({
      launchWindowConfirmationStatus: "confirmed",
      goLiveHoldStatus: "ready",
    ...
    expect(goLiveHoldReleaseAuthorizationReceipt.counts.goLiveHoldReleaseAuthorizationReceiptCheckedCount).toBe(1);
    ...
    expect(operationsHandoffAcceptanceReceipt).toMatchObject({
    ...
    expect(operationsHandoffAcceptanceReceipt.safeFilename).toBe("provider-webhook-review-qa-handoff-certified-release-operations-handoff-acceptance-receipt.json");
    expect(operationsHandoffAcceptanceReceipt.operationsHandoffAcceptanceReceiptDigest).toBe(operationsHandoffAcceptanceReceipt.safeDigest);
    expect(operationsHandoffAcceptanceReceipt.operationsHandoffEvidencePacketDigest).toBe(operationsHandoffReadinessPacket.operationsHandoffEvidencePacketDigest);
    expect(operationsHandoffAcceptanceReceipt.inheritedOperationsHandoffReadinessPacketSummary).toMatchObject({
      operationsHandoffReadinessStatus: "ready_for_handoff",
      operationsHandoffEvidencePacketStatus: "issued",
      noExecutionEvidenceStatus: "confirmed",
      launchApprovalLockStatus: "locked",
    ...
    expect(operationsHandoffAcceptanceReceipt.operationsHandoffAcceptanceRows.every((entry) => entry.complete && entry.status === "confirmed")).toBe(true);
    expect(operationsHandoffAcceptanceReceipt.operationsCustodyRows.every((entry) => entry.complete && entry.status === "confirmed")).toBe(true);
    expect(operationsHandoffAcceptanceReceipt.counts.operationsHandoffAcceptanceCheckedCount).toBe(1);
    expect(operationsHandoffAcceptanceReceipt.counts.operationsHandoffAcceptanceMutationCount).toBe(0);
    expect(operationsHandoffAcceptanceReceipt.counts.operationsHandoffAcceptanceAcceptedCount).toBe(operationsHandoffAcceptanceReceipt.operationsHandoffAcceptanceRows.length);
    expect(operationsHandoffAcceptanceReceipt.counts.operationsCustodyAcceptedCount).toBe(operationsHandoffAcceptanceReceipt.operationsCustodyRows.length);
    expect(operationsHandoffAcceptanceReceipt.counts.executionAttemptCount).toBe(0);
    expect(operationsHandoffAcceptanceReceipt.counts.providerOutboundCallCount).toBe(0);
    expect(operationsHandoffAcceptanceReceipt.counts.externalNotificationSendCount).toBe(0);
    expect(operationsHandoffAcceptanceReceipt.counts.aiCallCount).toBe(0);
    expect(operationsCustodyMonitoringReadinessLedger).toMatchObject({
    ...
    expect(operationsCustodyMonitoringReadinessLedger.safeFilename).toBe("provider-webhook-review-qa-handoff-certified-release-operations-custody-monitoring-readiness-ledger.json");
    expect(operationsCustodyMonitoringReadinessLedger.operationsCustodyMonitoringLedgerDigest).toBe(operationsCustodyMonitoringReadinessLedger.safeDigest);
    expect(operationsCustodyMonitoringReadinessLedger.operationsHandoffAcceptanceReceiptDigest).toBe(operationsHandoffAcceptanceReceipt.operationsHandoffAcceptanceReceiptDigest);
    expect(operationsCustodyMonitoringReadinessLedger.inheritedOperationsHandoffAcceptanceReceiptSummary).toMatchObject({
      operationsHandoffAcceptanceStatus: "accepted",
      operationsCustodyStatus: "accepted",
      noExecutionEvidenceStatus: "confirmed",
      launchApprovalLockStatus: "locked",
    ...
    expect(operationsCustodyMonitoringReadinessLedger.operationsCustodyMonitoringRows.every((entry) => entry.complete && entry.status === "confirmed")).toBe(true);
    expect(operationsCustodyMonitoringReadinessLedger.noExecutionMonitoringRows.every((entry) => entry.complete && entry.status === "confirmed")).toBe(true);
    expect(operationsCustodyMonitoringReadinessLedger.counts.operationsCustodyMonitoringCheckedCount).toBe(1);
    expect(operationsCustodyMonitoringReadinessLedger.counts.operationsCustodyMonitoringMutationCount).toBe(0);
    expect(operationsCustodyMonitoringReadinessLedger.counts.operationsCustodyMonitoringReadyCount).toBe(operationsCustodyMonitoringReadinessLedger.operationsCustodyMonitoringRows.length);
    expect(operationsCustodyMonitoringReadinessLedger.counts.noExecutionMonitoringActiveCount).toBe(operationsCustodyMonitoringReadinessLedger.noExecutionMonitoringRows.length);
    expect(operationsCustodyMonitoringReadinessLedger.counts.executionAttemptCount).toBe(0);
    expect(operationsCustodyMonitoringReadinessLedger.counts.providerOutboundCallCount).toBe(0);
    expect(operationsCustodyMonitoringReadinessLedger.counts.externalNotificationSendCount).toBe(0);
    expect(operationsCustodyMonitoringReadinessLedger.counts.aiCallCount).toBe(0);
    expect(operationsCustodyMonitoringCloseoutSealReceipt).toMatchObject({
    ...
    expect(operationsCustodyMonitoringCloseoutSealReceipt.safeFilename).toBe("provider-webhook-review-qa-handoff-certified-release-operations-custody-monitoring-closeout-seal-receipt.json");
    expect(operationsCustodyMonitoringCloseoutSealReceipt.operationsCustodyMonitoringCloseoutSealReceiptDigest).toBe(operationsCustodyMonitoringCloseoutSealReceipt.safeDigest);
    expect(operationsCustodyMonitoringCloseoutSealReceipt.operationsCustodyMonitoringLedgerDigest).toBe(operationsCustodyMonitoringReadinessLedger.operationsCustodyMonitoringLedgerDigest);
    expect(operationsCustodyMonitoringCloseoutSealReceipt.inheritedOperationsCustodyMonitoringReadinessLedgerSummary).toMatchObject({
      operationsCustodyMonitoringStatus: "ready",
      operationsHandoffAcceptanceStatus: "accepted",
      operationsCustodyStatus: "accepted",
      noExecutionEvidenceStatus: "confirmed",
    ...
    expect(operationsCustodyMonitoringCloseoutSealReceipt.operationsCustodyMonitoringCloseoutRows.every((entry) => entry.complete && entry.status === "confirmed")).toBe(true);
    expect(operationsCustodyMonitoringCloseoutSealReceipt.counts.operationsCustodyMonitoringCloseoutCheckedCount).toBe(1);
    expect(operationsCustodyMonitoringCloseoutSealReceipt.counts.operationsCustodyMonitoringCloseoutSealMutationCount).toBe(0);
    expect(operationsCustodyMonitoringCloseoutSealReceipt.counts.operationsCustodyMonitoringCloseoutSealedCount).toBe(operationsCustodyMonitoringCloseoutSealReceipt.operationsCustodyMonitoringCloseoutRows.length);
    expect(operationsCustodyMonitoringCloseoutSealReceipt.counts.executionAttemptCount).toBe(0);
    expect(operationsCustodyMonitoringCloseoutSealReceipt.counts.providerOutboundCallCount).toBe(0);
    expect(operationsCustodyMonitoringCloseoutSealReceipt.counts.externalNotificationSendCount).toBe(0);
    expect(operationsCustodyMonitoringCloseoutSealReceipt.counts.aiCallCount).toBe(0);
    expect(finalNoExecutionEvidenceRollup).toMatchObject({
    ...
    expect(finalNoExecutionEvidenceRollup.safeFilename).toBe("provider-webhook-review-qa-handoff-certified-release-final-no-execution-evidence-rollup.json");
    expect(finalNoExecutionEvidenceRollup.finalNoExecutionEvidenceRollupDigest).toBe(finalNoExecutionEvidenceRollup.safeDigest);
    expect(finalNoExecutionEvidenceRollup.operationsCustodyMonitoringCloseoutSealReceiptDigest).toBe(operationsCustodyMonitoringCloseoutSealReceipt.operationsCustodyMonitoringCloseoutSealReceiptDigest);
    expect(finalNoExecutionEvidenceRollup.inheritedOperationsCustodyMonitoringCloseoutSealReceiptSummary).toMatchObject({
      operationsCustodyMonitoringCloseoutStatus: "sealed",
      closeoutSealStatus: "sealed",
      operationsCustodyMonitoringStatus: "ready",
      operationsHandoffAcceptanceStatus: "accepted",
    ...
    expect(finalNoExecutionEvidenceRollup.finalNoExecutionEvidenceRows.every((entry) => entry.complete && entry.status === "confirmed")).toBe(true);
    expect(finalNoExecutionEvidenceRollup.counts.finalNoExecutionEvidenceRollupCheckedCount).toBe(1);
    expect(finalNoExecutionEvidenceRollup.counts.finalNoExecutionEvidenceRollupMutationCount).toBe(0);
    expect(finalNoExecutionEvidenceRollup.counts.finalNoExecutionEvidenceRollupIssuedCount).toBe(finalNoExecutionEvidenceRollup.finalNoExecutionEvidenceRows.length);
    expect(finalNoExecutionEvidenceRollup.counts.finalArchiveCustodySealedCount).toBe(1);
    expect(finalNoExecutionEvidenceRollup.counts.executionAttemptCount).toBe(0);
    expect(finalNoExecutionEvidenceRollup.counts.providerOutboundCallCount).toBe(0);
    expect(finalNoExecutionEvidenceRollup.counts.externalNotificationSendCount).toBe(0);
    expect(finalNoExecutionEvidenceRollup.counts.aiCallCount).toBe(0);
    expect(finalEvidenceIndexRegressionGuardrailReceipt).toMatchObject({
    ...
    expect(finalEvidenceIndexRegressionGuardrailReceipt.safeFilename).toBe("provider-webhook-review-qa-handoff-certified-release-final-evidence-index-regression-guardrail-receipt.json");
    expect(finalEvidenceIndexRegressionGuardrailReceipt.finalEvidenceIndexDigest).toBe(finalEvidenceIndexRegressionGuardrailReceipt.safeDigest);
    expect(finalEvidenceIndexRegressionGuardrailReceipt.regressionGuardrailReceiptDigest).toBe(finalEvidenceIndexRegressionGuardrailReceipt.safeDigest);
    expect(finalEvidenceIndexRegressionGuardrailReceipt.finalNoExecutionEvidenceRollupDigest).toBe(finalNoExecutionEvidenceRollup.finalNoExecutionEvidenceRollupDigest);
    expect(finalEvidenceIndexRegressionGuardrailReceipt.finalEvidenceIndexRows.map((entry) => entry.sprintNumber)).toEqual([103, 104, 105, 106, 107, 108, 109, 110]);
    expect(finalEvidenceIndexRegressionGuardrailReceipt.finalEvidenceIndexRows.every((entry) => entry.externalCalls === 0 && entry.mutationCount === 0)).toBe(true);
    expect(finalEvidenceIndexRegressionGuardrailReceipt.inheritedFinalNoExecutionEvidenceRollupSummary).toMatchObject({
      finalNoExecutionEvidenceRollupStatus: "issued",
      finalArchiveCustodyStatus: "sealed",
    ...
    expect(finalEvidenceIndexRegressionGuardrailReceipt.counts.finalEvidenceIndexMutationCount).toBe(0);
    expect(finalEvidenceIndexRegressionGuardrailReceipt.counts.regressionGuardrailMutationCount).toBe(0);
    expect(finalEvidenceIndexRegressionGuardrailReceipt.counts.finalNoExecutionEvidenceRollupMutationCount).toBe(0);
    expect(finalEvidenceIndexRegressionGuardrailReceipt.counts.executionAttemptCount).toBe(0);
    expect(finalEvidenceIndexRegressionGuardrailReceipt.counts.providerOutboundCallCount).toBe(0);
    expect(finalEvidenceIndexRegressionGuardrailReceipt.counts.externalNotificationSendCount).toBe(0);
    expect(finalEvidenceIndexRegressionGuardrailReceipt.counts.aiCallCount).toBe(0);
    expect(finalEvidenceIndexRegressionGuardrailReceiptReadback).toMatchObject({
      receiptKind: finalEvidenceIndexRegressionGuardrailReceipt.receiptKind,
    ...
    expect(finalArchiveSealOperationalClosureReceipt).toMatchObject({
    ...
    expect(finalArchiveSealOperationalClosureReceipt.safeFilename).toBe("provider-webhook-review-qa-handoff-certified-release-final-archive-seal-operational-closure-receipt.json");
    expect(finalArchiveSealOperationalClosureReceipt.finalOperationalClosureReceiptDigest).toBe(finalArchiveSealOperationalClosureReceipt.safeDigest);
    expect(finalArchiveSealOperationalClosureReceipt.finalArchiveSealDigest).toBe(finalArchiveSealOperationalClosureReceipt.safeDigest);
    expect(finalArchiveSealOperationalClosureReceipt.finalEvidenceIndexDigest).toBe(finalEvidenceIndexRegressionGuardrailReceipt.finalEvidenceIndexDigest);
    expect(finalArchiveSealOperationalClosureReceipt.regressionGuardrailReceiptDigest).toBe(finalEvidenceIndexRegressionGuardrailReceipt.regressionGuardrailReceiptDigest);
    expect(finalArchiveSealOperationalClosureReceipt.finalArchiveSealOperationalClosureRows.map((entry) => entry.sprintNumber)).toEqual([103, 104, 105, 106, 107, 108, 109, 110, 111]);
    expect(finalArchiveSealOperationalClosureReceipt.finalArchiveSealOperationalClosureRows.every((entry) => entry.externalCalls === 0 && entry.mutationCount === 0)).toBe(true);
    expect(finalArchiveSealOperationalClosureReceipt.inheritedFinalEvidenceIndexRegressionGuardrailReceiptSummary).toMatchObject({
      finalEvidenceIndexStatus: "issued",
      regressionGuardrailReceiptStatus: "issued",
      regressionGuardrailStatus: "passed",
    ...
    expect(finalArchiveSealOperationalClosureReceipt.counts.finalOperationalClosureReceiptMutationCount).toBe(0);
    expect(finalArchiveSealOperationalClosureReceipt.counts.finalArchiveSealMutationCount).toBe(0);
    expect(finalArchiveSealOperationalClosureReceipt.counts.finalEvidenceIndexMutationCount).toBe(0);
    expect(finalArchiveSealOperationalClosureReceipt.counts.regressionGuardrailMutationCount).toBe(0);
    expect(finalArchiveSealOperationalClosureReceipt.counts.finalNoExecutionEvidenceRollupMutationCount).toBe(0);
    expect(finalArchiveSealOperationalClosureReceipt.counts.executionAttemptCount).toBe(0);
    expect(finalArchiveSealOperationalClosureReceipt.counts.providerOutboundCallCount).toBe(0);
    expect(finalArchiveSealOperationalClosureReceipt.counts.externalNotificationSendCount).toBe(0);
    expect(finalArchiveSealOperationalClosureReceipt.counts.aiCallCount).toBe(0);
    expect(finalArchiveSealOperationalClosureReceiptReadback).toMatchObject({
      receiptKind: finalArchiveSealOperationalClosureReceipt.receiptKind,
    ...
    expect(postClosurePreservationContinuityLedgerReceipt).toMatchObject({
    ...
    expect(postClosurePreservationContinuityLedgerReceipt.safeFilename).toBe("provider-webhook-certified-release-post-closure-preservation-continuity-ledger-receipt.json");
    expect(postClosurePreservationContinuityLedgerReceipt.postClosurePreservationContinuityLedgerDigest).toBe(postClosurePreservationContinuityLedgerReceipt.safeDigest);
    expect(postClosurePreservationContinuityLedgerReceipt.postClosurePreservationVerificationDigest).toBe(postClosurePreservationVerificationReceipt.safeDigest);
    expect(postClosurePreservationContinuityLedgerReceipt.preservationContinuityLedgerRows.map((entry) => entry.sprintNumber)).toEqual([103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113]);
    expect(postClosurePreservationContinuityLedgerReceipt.preservationContinuityLedgerRows.every((entry) => entry.externalCalls === 0 && entry.mutationCount === 0)).toBe(true);
    expect(postClosurePreservationContinuityLedgerReceipt.inheritedPostClosurePreservationVerificationReceiptSummary).toMatchObject({
      postClosurePreservationVerificationStatus: "verified",
      finalArchiveSealPostClosurePreservationStatus: "preserved",
    ...
    expect(postClosurePreservationContinuityLedgerReceipt.counts.preservationContinuityLedgerMutationCount).toBe(0);
    expect(postClosurePreservationContinuityLedgerReceipt.counts.postClosurePreservationVerificationMutationCount).toBe(0);
    expect(postClosurePreservationContinuityLedgerReceipt.counts.executionAttemptCount).toBe(0);
    expect(postClosurePreservationContinuityLedgerReceipt.counts.providerOutboundCallCount).toBe(0);
    expect(postClosurePreservationContinuityLedgerReceipt.counts.externalNotificationSendCount).toBe(0);
    expect(postClosurePreservationContinuityLedgerReceipt.counts.aiCallCount).toBe(0);
    expect(postClosurePreservationContinuityLedgerReceiptReadback).toMatchObject({
      receiptKind: postClosurePreservationContinuityLedgerReceipt.receiptKind,
    ...
    expect(postClosurePreservationCustodyAuditReceipt).toMatchObject({
    ...
    expect(postClosurePreservationCustodyAuditReceipt.safeFilename).toBe("provider-webhook-certified-release-post-closure-preservation-custody-audit-receipt.json");
    expect(postClosurePreservationCustodyAuditReceipt.postClosurePreservationCustodyAuditDigest).toBe(postClosurePreservationCustodyAuditReceipt.safeDigest);
    expect(postClosurePreservationCustodyAuditReceipt.postClosurePreservationContinuityLedgerDigest).toBe(postClosurePreservationContinuityLedgerReceipt.safeDigest);
    expect(postClosurePreservationCustodyAuditReceipt.custodyAuditRows.map((entry) => entry.sprintNumber)).toEqual([103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114]);
    expect(postClosurePreservationCustodyAuditReceipt.custodyAuditRows.every((entry) => entry.externalCalls === 0 && entry.mutationCount === 0 && entry.custodyAuditStatus === "under_safe_custody")).toBe(true);
    expect(postClosurePreservationCustodyAuditReceipt.inheritedPostClosurePreservationContinuityLedgerReceiptSummary).toMatchObject({
      postClosurePreservationContinuityLedgerStatus: "continuous",
      postClosurePreservationVerificationStatus: "verified",
    ...
    expect(postClosurePreservationCustodyAuditReceipt.counts.postClosurePreservationCustodyAuditMutationCount).toBe(0);
    expect(postClosurePreservationCustodyAuditReceipt.counts.preservationContinuityLedgerMutationCount).toBe(0);
    expect(postClosurePreservationCustodyAuditReceipt.counts.executionAttemptCount).toBe(0);
    expect(postClosurePreservationCustodyAuditReceipt.counts.providerOutboundCallCount).toBe(0);
    expect(postClosurePreservationCustodyAuditReceipt.counts.externalNotificationSendCount).toBe(0);
    expect(postClosurePreservationCustodyAuditReceipt.counts.aiCallCount).toBe(0);
    expect(postClosurePreservationCustodyAuditReceiptReadback).toMatchObject({
      receiptKind: postClosurePreservationCustodyAuditReceipt.receiptKind,
    ...
    expect(postClosurePreservationCustodyChainSealReceipt).toMatchObject({
    ...
    expect(postClosurePreservationCustodyChainSealReceipt.safeFilename).toBe("provider-webhook-certified-release-post-closure-preservation-custody-chain-seal-receipt.json");
    expect(postClosurePreservationCustodyChainSealReceipt.postClosurePreservationCustodyChainSealDigest).toBe(postClosurePreservationCustodyChainSealReceipt.safeDigest);
    expect(postClosurePreservationCustodyChainSealReceipt.postClosurePreservationCustodyAuditDigest).toBe(postClosurePreservationCustodyAuditReceipt.safeDigest);
    expect(postClosurePreservationCustodyChainSealReceipt.custodyChainSealRows.map((entry) => entry.sprintNumber)).toEqual([103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115]);
    expect(postClosurePreservationCustodyChainSealReceipt.custodyChainSealRows.every((entry) => entry.externalCalls === 0 && entry.mutationCount === 0 && entry.custodyAuditStatus === "under_safe_custody" && entry.custodyChainSealStatus === "sealed_under_safe_custody")).toBe(true);
    expect(postClosurePreservationCustodyChainSealReceipt.inheritedPostClosurePreservationCustodyAuditReceiptSummary).toMatchObject({
      postClosurePreservationCustodyAuditStatus: "audited",
      postClosurePreservationContinuityLedgerStatus: "continuous",
      postClosurePreservationVerificationStatus: "verified",
    ...
    expect(postClosurePreservationCustodyChainSealReceipt.counts.postClosurePreservationCustodyChainSealMutationCount).toBe(0);
    expect(postClosurePreservationCustodyChainSealReceipt.counts.postClosurePreservationCustodyAuditMutationCount).toBe(0);
    expect(postClosurePreservationCustodyChainSealReceipt.counts.executionAttemptCount).toBe(0);
    expect(postClosurePreservationCustodyChainSealReceipt.counts.providerOutboundCallCount).toBe(0);
    expect(postClosurePreservationCustodyChainSealReceipt.counts.externalNotificationSendCount).toBe(0);
    expect(postClosurePreservationCustodyChainSealReceipt.counts.aiCallCount).toBe(0);
    expect(postClosurePreservationCustodyChainSealReceiptReadback).toMatchObject({
      receiptKind: postClosurePreservationCustodyChainSealReceipt.receiptKind,
    ...
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt).toMatchObject({
    ...
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.safeFilename).toBe("provider-webhook-certified-release-post-closure-preservation-custody-chain-integrity-ledger-continuity-receipt.json");
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.postClosurePreservationCustodyChainIntegrityLedgerContinuityDigest).toBe(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.safeDigest);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.sprint116ReceiptReference.safeDigest).toBe(postClosurePreservationCustodyChainIntegrityLedgerReceipt.safeDigest);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.sealedArchiveReference.safeDigest).toBe(postClosurePreservationCustodyChainSealReceipt.safeDigest);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.safeRowSummaries.map((entry) => entry.sprintNumber)).toEqual([103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117]);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.safeRowSummaries.every((entry) => entry.externalCalls === 0 && entry.mutationCount === 0 && entry.custodyAuditStatus === "under_safe_custody" && entry.custodyChainSealStatus === "sealed_under_safe_custody" && entry.custodyChainIntegrityLedgerStatus === "integrity_confirmed_under_safe_custody" && entry.custodyChainIntegrityLedgerContinuityStatus === "continuity_confirmed_under_safe_custody")).toBe(true);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.counts.custodyChainIntegrityLedgerContinuityRowCount).toBe(15);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.counts.custodyChainIntegrityLedgerContinuitySafeCount).toBe(15);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.noExecutionFlags).toMatchObject({
      externalCallsZero: true,
      executionAttemptCount: 0,
      providerOutboundCallCount: 0,
      externalNotificationSendCount: 0,
    ...
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceiptReadback).toMatchObject({
      receiptKind: postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.receiptKind,
      postClosurePreservationCustodyChainIntegrityLedgerContinuityStatus: postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.postClosurePreservationCustodyChainIntegrityLedgerContinuityStatus,
      sprint116ReceiptReference: {
        safeDigest: postClosurePreservationCustodyChainIntegrityLedgerReceipt.safeDigest
      },
      externalCalls: 0
    });
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt).toMatchObject({
    ...
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt.safeFilename).toBe("provider-webhook-certified-release-custody-chain-integrity-ledger-continuity-verification-receipt.json");
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt.continuityVerificationDigest).toBe(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt.safeDigest);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt.derivedFrom.safeDigest).toBe(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.safeDigest);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt.sprint117ReceiptDigest).toBe(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.safeDigest);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt.verificationRows.map((entry) => entry.sprintNumber)).toEqual([103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118]);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt.verificationRows.every((entry) => entry.externalCalls === 0 && entry.mutationCount === 0 && entry.custodyChainStatus === "sealed_under_safe_custody" && entry.ledgerIntegrityStatus === "integrity_confirmed_under_safe_custody" && entry.continuityStatus === "continuity_confirmed_under_safe_custody" && entry.verificationStatus === "verified_under_safe_custody")).toBe(true);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt.counts.verificationRowCount).toBe(16);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt.counts.verificationSafeCount).toBe(16);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt.noExecutionFlags).toMatchObject({
      externalCallsZero: true,
      executionAttemptCount: 0,
      providerOutboundCallCount: 0,
      externalNotificationSendCount: 0,
    ...
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceiptReadback).toMatchObject({
    ...
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt).toMatchObject({
    ...
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt.safeFilename).toBe("provider-webhook-certified-release-custody-chain-integrity-ledger-continuity-verification-audit-receipt.json");
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt.continuityVerificationAuditDigest).toBe(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt.safeDigest);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt.derivedFrom.safeDigest).toBe(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt.safeDigest);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt.sprint118ReceiptDigest).toBe(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt.safeDigest);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt.sprint117ReceiptDigest).toBe(postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt.safeDigest);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt.auditRows.map((entry) => entry.sprintNumber)).toEqual([103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119]);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt.auditRows.every((entry) => entry.externalCalls === 0 && entry.mutationCount === 0 && entry.custodyChainStatus === "sealed_under_safe_custody" && entry.ledgerIntegrityStatus === "integrity_confirmed_under_safe_custody" && entry.continuityStatus === "continuity_confirmed_under_safe_custody" && entry.verificationStatus === "verified_under_safe_custody" && entry.auditStatus === "audited_under_safe_custody")).toBe(true);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt.counts.auditRowCount).toBe(17);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt.counts.auditSafeCount).toBe(17);
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt.noExecutionFlags).toMatchObject({
      externalCallsZero: true,
      executionAttemptCount: 0,
      providerOutboundCallCount: 0,
      externalNotificationSendCount: 0,
    ...
    expect(postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceiptReadback).toMatchObject({
    ...
    vi.spyOn(service, "getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceipt")
      .mockReturnValueOnce({ ...postClosurePreservationCustodyChainIntegrityLedgerReceipt, postClosurePreservationCustodyChainIntegrityLedgerStatus: "incomplete" });
    expect(() => service.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt(tenantId, filters, "operator-current"))
      .toThrow("post-closure preservation custody chain integrity ledger continuity receipt prerequisites are incomplete");
    vi.spyOn(service, "getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerReceipt")
      .mockReturnValueOnce({ ...postClosurePreservationCustodyChainIntegrityLedgerReceipt, externalCalls: 1 as 0 });
    expect(() => service.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt(tenantId, filters, "operator-current"))
      .toThrow("post-closure preservation custody chain integrity ledger continuity receipt prerequisites are incomplete");
    vi.spyOn(service, "getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt")
      .mockReturnValueOnce({ ...postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt, postClosurePreservationCustodyChainIntegrityLedgerContinuityStatus: "incomplete" });
    expect(() => service.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt(tenantId, filters, "operator-current"))
      .toThrow("post-closure preservation custody chain integrity ledger continuity verification receipt prerequisites are incomplete");
    vi.spyOn(service, "getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt")
      .mockReturnValueOnce({ ...postClosurePreservationCustodyChainIntegrityLedgerContinuityReceipt, externalCalls: 1 as 0 });
    expect(() => service.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt(tenantId, filters, "operator-current"))
      .toThrow("post-closure preservation custody chain integrity ledger continuity verification receipt prerequisites are incomplete");
    vi.spyOn(service, "getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt")
      .mockReturnValueOnce({ ...postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt, receiptStatus: "incomplete" });
    expect(() => service.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt(tenantId, filters, "operator-current"))
      .toThrow("post-closure preservation custody chain integrity ledger continuity verification audit receipt prerequisites are incomplete");
    vi.spyOn(service, "getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt")
      .mockReturnValueOnce({ ...postClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceipt, externalCalls: 1 as 0 });
    expect(() => service.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceipt(tenantId, filters, "operator-current"))
      .toThrow("post-closure preservation custody chain integrity ledger continuity verification audit receipt prerequisites are incomplete");
    ...
    expect(() => service.getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyAuditReceipt(tenantId, filters, "operator-current"))
      .toThrow("post-closure preservation custody audit receipt prerequisites are incomplete");
    vi.spyOn(service, "getReviewQaHandoffCertifiedReleaseFinalArchiveSealPostClosurePreservationCustodyAuditReceipt")
    ...
    expect(beforePostClosurePreservationCustodyChainIntegrityLedgerContinuityReceiptRead).toEqual(afterPostClosurePreservationCustodyChainIntegrityLedgerContinuityReceiptRead);
    expect(beforePostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceiptRead).toEqual(afterPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationReceiptRead);
    expect(beforePostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceiptRead).toEqual(afterPostClosurePreservationCustodyChainIntegrityLedgerContinuityVerificationAuditReceiptRead);
    ...
    expect(beforeOperationsHandoffAcceptanceReceiptRead).toEqual(afterOperationsHandoffAcceptanceReceiptRead);
    expect(beforeOperationsCustodyMonitoringReadinessLedgerRead).toEqual(afterOperationsCustodyMonitoringReadinessLedgerRead);
    expect(beforeOperationsCustodyMonitoringCloseoutSealReceiptRead).toEqual(afterOperationsCustodyMonitoringCloseoutSealReceiptRead);
    ...
    expect(afterGoLiveAuthorizationReceiptRead).toMatchObject({
      reviewStatus: beforeGoLiveAuthorizationReceiptRead?.reviewStatus,
      linkStatus: beforeGoLiveAuthorizationReceiptRead?.linkStatus,
      unmatchedStatus: beforeGoLiveAuthorizationReceiptRead?.unmatchedStatus,
      assignmentStatus: beforeGoLiveAuthorizationReceiptRead?.assignmentStatus,
      escalationStatus: beforeGoLiveAuthorizationReceiptRead?.escalationStatus,
      resolutionStatus: beforeGoLiveAuthorizationReceiptRead?.resolutionStatus,
    ...
    expect(afterGoLiveHoldReleaseAuthorizationReceiptRead).toMatchObject({
      reviewStatus: beforeGoLiveHoldReleaseAuthorizationReceiptRead?.reviewStatus,
      linkStatus: beforeGoLiveHoldReleaseAuthorizationReceiptRead?.linkStatus,
      unmatchedStatus: beforeGoLiveHoldReleaseAuthorizationReceiptRead?.unmatchedStatus,
      assignmentStatus: beforeGoLiveHoldReleaseAuthorizationReceiptRead?.assignmentStatus,
      escalationStatus: beforeGoLiveHoldReleaseAuthorizationReceiptRead?.escalationStatus,
      resolutionStatus: beforeGoLiveHoldReleaseAuthorizationReceiptRead?.resolutionStatus,
